            ...
            <tbody>
                @foreach($szms as $s)
                    <tr>
                        ...
                        <td>{{$s->tuijianren}}</td>
                        <td>{{$s->szm}}</td>
                        <td>{{date('m-d',$s->created_at)}}</td>
                        <td>{{date('m-d',$s->updated_at)}}</td>
                ...
                <tr>
                    ...
                    <td colspan="3">金额(元):{{$curr['curr_money']}}</td>
                    <td colspan="3">已结佣金(元):{{$curr['curr_commission1']}}</td>
                    <td colspan="3">未结佣金(元):{{$curr['curr_commission0']}}</td>
                </tr>
            </tbody>
        </table>
        {!! $szms->links() !!}
